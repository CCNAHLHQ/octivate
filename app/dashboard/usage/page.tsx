          ...
          <div className="grid gap-5 sm:grid-cols-2">
            <Card className="p-6">
              <h2 className="font-mono text-[11px] font-bold uppercase tracking-widest text-faint">
                Token allowance · {usage.period}
              </h2>
              <div className="mt-4 font-display text-4xl font-bold tracking-tight text-foam">
                {usage.tokensUsed.toLocaleString()}
                <span className="text-xl font-semibold text-mist"> / {usage.tokensLimit.toLocaleString()}</span>
              </div>
              <ProgressBar className="mt-5" value={pct} />
              <p className="mt-4 text-sm text-mist">
                Estimated cost{" "}
                <span className="font-mono font-bold text-teal">${usage.estimatedCostUsd.toFixed(2)}</span>
              </p>
            </Card>
