...
export type BillingCheckoutFormProps = {
...
export function BillingCheckoutForm({
  ...
  }, [context.planId, plans]);

  const price = useMemo(() => {
    if (!plan) return null;
    return resolvePrice(plan, context.interval);
  }, [plan, context.interval]);
  ...
  useEffect(() => {
    ...
    setPromoInput("");
  }, [context.planId, context.interval, context.methodId]);

  useEffect(() => {
    if (methodId !== "card") setCardDetailsOpen(false);
  }, [methodId]);

  /* Escape closes email overlay only (no page body lock) */
  useEffect(() => {
    if (!addEmailOpen) return;
    const onKey = (e: KeyboardEvent) => {
      if (e.key === "Escape") setAddEmailOpen(false);
    };
  ...
  const continueHref = user ? "/dashboard" : "/signup";
...
/** @deprecated Prefer BillingCheckoutForm — alias for backward compatibility. */
export const BillingCheckout = BillingCheckoutForm;
